# -*- coding: utf-8 -*-
"""
Created on Wed Feb  7 02:17:19 2024
...
'''
'''
# Selecciona algunas características relevantes para X y la etiqueta para y
features = ['Flow Duration', 'Tot Fwd Pkts', 'Tot Bwd Pkts', 'TotLen Fwd Pkts', 
                    'TotLen Bwd Pkts', 'Fwd Pkt Len Max', 'Fwd Pkt Len Min', 'Fwd Pkt Len Mean', 'Fwd Pkt Len Std', 
                    'Bwd Pkt Len Max', 'Bwd Pkt Len Min', 'Bwd Pkt Len Mean', 'Bwd Pkt Len Std', 'Flow IAT Mean',
                    'Flow IAT Std', 'Flow IAT Max', 'Flow IAT Min', 'Fwd IAT Tot', 'Fwd IAT Mean', 'Fwd IAT Std', 
...
'''
'''
#Las 20 características más importantes para este data este dataset 2, son: 
features = ['Init Bwd Win Byts',
                'Flow IAT Min',
                'Flow IAT Max',
                'Flow Duration',
                'Bwd Header Len',
...
'''
# Ajustar el parámetro de sampling_strategy para el sobremuestreo con SMOTE
over_sampling_strategy = SMOTE(sampling_strategy=0.5)  # Ajusta la proporción de la clase minoritaria al 50% de la clase mayoritaria

# Crear el pipeline solo con el sobremuestreo
pipeline = Pipeline([('over_sampling', over_sampling_strategy),
                     ('model', modelo_rf)])

'''
'''
# Definir la estrategia de submuestreo
under_sampling_strategy = RandomUnderSampler(sampling_strategy=1.0)  # Igualar el número de muestras de ambas clases

# Crear el pipeline solo con submuestreo
pipeline = Pipeline([('under_sampling', under_sampling_strategy),
                     ('model', modelo_rf)])
'''
...
# Iniciar temporizador para la ejecución
...
# Imprimir los tiempos de entrenamiento y ejecución
...
print(f"Precisión del modelo Random Forest después de aplicar sobre muestreo y submuestreo: {precision_rf}")
...
print("Las 20 características más importantes son:")
